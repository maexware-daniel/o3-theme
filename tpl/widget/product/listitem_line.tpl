[{block name="widget_product_listitem_line"}]
    ...
    [{* Should items in the listing be able to be added to the shopping cart? YOU NEED THIS! *}]
    [{*
    ...
    <div class="row">
        [{block name="widget_product_listitem_line_picturebox"}]
            <a class="component__productbox-picture col-4 col-md-3 h-auto" href="[{$_productLink}]" title="[{$product->oxarticles__oxtitle->value}] [{$product->oxarticles__oxvarselect->value}]">
                <img loading="lazy" src="[{$product->getThumbnailUrl()}]" alt="[{$product->oxarticles__oxtitle->value}] [{$product->oxarticles__oxvarselect->value}]">
            </a>
        [{/block}]

        <div class="col-8 col-md-5 d-flex flex-column justify-content-between py-md-4 row-gap-3">
            ...
            [{block name="widget_product_listitem_line_description"}]
                <div class="component__productbox-description d-none d-md-block">
                    [{if $recommid}]
                        <p>[{$product->text}]</p>
                    [{else}]
        ...
        [{block name="widget_product_listitem_line_attributes"}]
            <div class="component__productbox-attributes d-none d-md-block col-3 py-4">
                [{assign var="oAttributes" value=$product->getAttributes()}]
                [{if $oAttributes|@count}]
                    [{oxmultilang ident="SPECIFICATION" suffix="COLON"}]
                    <ul>
                        [{foreach from=$oAttributes item="oAttr" name="attribute"}]
                        <li>
                            <span>[{$oAttr->oxattribute__oxtitle->value}][{oxmultilang ident="COLON"}]</span>
